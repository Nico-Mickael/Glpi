...
class Consumable extends CommonDBChild
{
    ...
    /**
     * Return the consumable to stock, saving usage history and applying the selected state.
     *
     * @param array $input Must contain 'id'. May contain 'consumablestates_id' (default 2 = En stock).
     * @param bool $history Not used (kept for signature compatibility)
     *
     * @return bool
     */
    public function backToStock(array $input, $history = true)
    {
        ...
        // 1. Load current state BEFORE updating (for history logging)
        $current = $this->fields;
        if (empty($current) || !$this->getFromDB($input['id'])) {
            return false;
        }
        $current = $this->fields;

        $consumablestates_id = (int) ($input['consumablestates_id'] ?? 2);
        $motif               = trim((string) ($input['motif'] ?? ''));
        $return_date         = date('Y-m-d H:i:s');

        // 2. Log the usage session that just ended (only if assigned to someone)
        if (!empty($current['itemtype']) && !empty($current['items_id'])) {
            $DB->insert('glpi_consumablelogs', [
                'consumables_id'      => $current['id'],
                'consumableitems_id'  => $current['consumableitems_id'],
                'itemtype'            => $current['itemtype'],
                'items_id'            => $current['items_id'],
                'date_out'            => $current['date_out'],
        ...
        // 3. Return consumable to stock with the selected state and return info.
        //    We keep itemtype/items_id (last assigned user) so the
        //    "Given to" column stays visible after reintegration.
        $result = $DB->update(
            static::getTable(),
            [
                'date_out'            => 'NULL',
    ...
    public static function showMassiveActionsSubForm(MassiveAction $ma)
    {
        ...
        switch ($ma->getAction()) {
            case 'backtostock':
                echo "<br>" . __('Nouvel état') . " ";
                Dropdown::show('ConsumableState', [
                    'name'  => 'consumablestates_id',
                    'value' => 2,
            ...
            case 'backtostock':
                foreach ($ids as $id) {
                    if ($item->can($id, UPDATE)) {
                        if ($item->backToStock([
                            'id'                  => $id,
                            'consumablestates_id' => $ma->getInput()['consumablestates_id'] ?? 2,
                            'motif'               => $ma->getInput()['motif'] ?? '',
                        ])) {
    ...
    public function rawSearchOptions()
    {
        ...
        // State of reusable consumable
        $options[] = [
            'id'                 => '90',
            'itemtype'           => ConsumableState::class,
            'table'              => ConsumableState::getTable(),
            'field'              => 'name',
            'name'               => __('État du consommable'),
            'datatype'           => 'dropdown',
            'linkfield'          => 'consumablestates_id',
        ];

        // Date of the last reintegration into stock
        $options[] = [
            'id'                 => '91',
            'table'              => static::getTable(),
            'field'              => 'date_return',
            'name'               => __('Date de réintégration'),
            'datatype'           => 'datetime',
        ];

        // Reason of the last reintegration
        $options[] = [
            'id'                 => '92',
            'table'              => static::getTable(),
            'field'              => 'motif',
            'name'               => __('Motif de réintégration'),
            'datatype'           => 'text',
        ];
